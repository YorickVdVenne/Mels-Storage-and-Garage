                ...
                <div className='w-full flex justify-center mt-8'>
                    <div className='grid grid-cols-10 gap-4'>
                        <div className='col-start-2 col-end-4'>
                            <ServiceCard onClick={() => { setOpenDialog(ServiceData.storage) }} buttonText='Storage' icon='warehouse'/>
                        </div>
                        <div className='col-start-5 col-end-7'>
                            <ServiceCard onClick={() => { setOpenDialog(ServiceData.maintenance) }} buttonText='Maintenance' icon='screwdriver-wrench'/>
                        </div>
                        <div className='col-start-8 col-end-10'>
                            <ServiceCard onClick={() => { setOpenDialog(ServiceData.others) }} buttonText='Others' icon='gear'/>
                        </div>
                    </div>
                </div>
                <div>
                    <Dialog 
                        open={typeof openDialog === 'string' && Object.values(ServiceData).includes(openDialog)} 
                        onCloseClick={() => { setOpenDialog(false) }}
                        data={openDialog !== false ? dataMapping[openDialog as ServiceData] : undefined}
                    />
                </div>
            </div>
        </div>
